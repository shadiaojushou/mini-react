...
let stateHookIndex
/**
 * @description: useState hooks API 用于函数组件中使用状态
 * @param {*} initial 初始值
 * @return {*} [state, setState] 返回状态和更新状态的函数
 */
function useState(initial) {
	...
	function setState(action) {
		...
		stateHook.queue.push(typeof action === 'function' ? action : () => action) // 将action存入队列中
...
/**
 * @description: useEffect hooks API 用于函数组件中使用副作用
...
function commitRoot() {
	...
	commitEffectHooks() // 在渲染完成后，执行副作用
	currentRoot = wipRoot
	wipRoot = null
	deletions = []
}
...
/**
 * @description: commitEffect 执行副作用 确保函数组件的副作用在渲染完成后执行
 * @return {*}
 */
function commitEffectHooks() {
	function run(fiber) {
		...
		if (!fiber.alternate) {
			...
			// 判断如果是更新时，检测deps是否发生变化，如果发生变化，就执行callback
